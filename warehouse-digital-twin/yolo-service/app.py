...
def publish(camera_id: str, objects: list, image_url: str = ""):
    payload = {
        ...
        "imageUrl": image_url,
        "objects": objects,
        "interaction": interaction_cache.get(camera_id),
    }
...
def stream_loop(camera_id: str, rtsp_url: str, stop: threading.Event):
    ...
    while not stop.is_set():
        ...
        if not ok:
            ...
            continue
        frame_no += 1
        if frame_no % int(CONFIG.get("process_every_n_frames", 3)):
            continue
        try:
            objects, annotated = get_detector().predict(frame)
            ok, encoded = cv2.imencode(".jpg", annotated)
            if ok:
                latest_frames[camera_id] = encoded.tobytes()
...
@app.get("/health")
def health():
    return {"status": "ok", "modelLoaded": detector is not None, "activeStreams": list(workers)}
...
def start_stream(body: StreamRequest):
    if body.cameraId in workers:
        return {"message": "already running"}
    stop = threading.Event()
    thread = threading.Thread(target=stream_loop, args=(body.cameraId, body.rtspUrl, stop), daemon=True)
    workers[body.cameraId] = (thread, stop)
    thread.start()
