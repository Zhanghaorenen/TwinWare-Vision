...
class YoloDetector:
    ...
    def __init__(self, weights: str, confidence=.35, iou=.45, image_size=640):
        path = Path(weights)
        if not path.is_absolute(): path = (Path(__file__).parent / path).resolve()
        self.model = (torch.hub.load(str(YOLO_ROOT), "custom", path=str(path), source="local") if path.exists()
    ...
    def predict(self, frame: np.ndarray):
        ...
        for x1, y1, x2, y2, confidence, cls in rows:
            ...
            objects.append({"className": name, "confidence": round(float(confidence), 4),
    ...
    def predict_bytes(self, content: bytes):
        ...
        if frame is None: raise ValueError("无法解析图片")
